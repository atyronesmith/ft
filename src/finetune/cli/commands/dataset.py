...
def prepare(
    ...
    template: str = typer.Option("alpaca", "--template", "-t", help="Template format (alpaca, chatml, llama)"),
    format: str = typer.Option("auto", "--format", "-f", help="Input format (auto, json, jsonl, csv, parquet)"),
    max_length: int = typer.Option(2048, "--max-length", help="Maximum sequence length"),
):
    """Prepare a dataset for training."""
    console.print(f"[bold cyan]📊 Preparing Dataset[/bold cyan]")
    ...
    if format == "auto":
        if input_path.suffix == ".json":
            ...
            console.print(f"[yellow]⚠️  Unknown format, assuming JSON[/yellow]")
            format = "json"
...
def validate(
    ...
    console.print(f"[bold cyan]🔍 Validating Dataset[/bold cyan]")
        ...
        if not isinstance(data, list):
            ...
            for i, item in enumerate(data[:100]):  # Check first 100
                ...
                if "text" not in item and ("instruction" not in item or "output" not in item):
                    issues.append(f"Example {i}: Missing required fields (text or instruction/output)")
...
def split(
    ...
    console.print(f"[bold cyan]✂️  Splitting Dataset[/bold cyan]")
    console.print(f"Input: {input_path}")
    console.print(f"Ratios - Train: {train_ratio:.0%}, Val: {val_ratio:.0%}, Test: {test_ratio:.0%}")
    ...
    try:
        ...
        for split_name, split_data in splits.items():
            if len(split_data) > 0:
                ...
                console.print(f"[green]✓ {split_name}: {len(split_data)} examples → {output_file}[/green]")
...
def stats(
    ...
    console.print(f"[bold cyan]📈 Dataset Statistics[/bold cyan]")
    console.print(f"Path: {dataset_path}\n")
...
def list_datasets(
    ...
    console.print(f"[bold cyan]📚 Available Datasets[/bold cyan]")
    ...
    for dataset_path in sorted(datasets):
        ...
        relative_path = dataset_path.relative_to(directory) if dataset_path.is_relative_to(directory) else dataset_path.name
        
        table.add_row(
            str(relative_path),
            dataset_path.suffix[1:],
            size_str,
            modified
        )
